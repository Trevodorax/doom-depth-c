...
char * test_save_player() {
    ...
    list_t *p = create_struct_from_db(db, sql_query, create_player_from_db, sizeof (player_t));
    player_t *player = (player_t *) p->head->value;
...
char * test_create_monsters_from_db() {
    sqlite3 *db = db_connection();
    list_t *monsters = create_struct_from_db(db, "SELECT * FROM MONSTER", create_monster_from_db, sizeof (monster_t));
    print_list(monsters, print_monster);
    mu_assert("Error in test_create_monsters_from_db: monsters is null", monsters != NULL);
    return 0;
}

void print_armor(void *armor) {
...
char * test_create_armors_from_db() {
    sqlite3 *db = db_connection();
    list_t *armors = create_struct_from_db(db, "SELECT * FROM ARMOR", create_armor_from_db, sizeof (armor_t));
    print_list(armors, print_armor);
    mu_assert("Error in test_create_armors_from_db: armors is null", armors != NULL);
    return 0;
}

void print_weapon(void *weapon) {
...
char * test_create_weapons_from_db() {
    sqlite3 *db = db_connection();
    list_t *weapons = create_struct_from_db(db, "SELECT * FROM WEAPON", create_weapon_from_db, sizeof (weapon_t));
    print_list(weapons, print_weapon);
    mu_assert("Error in test_create_weapons_from_db: weapons is null", weapons != NULL);
    return 0;
}

void print_spell(void *spell) {
...
char * test_create_spells_from_db() {
    sqlite3 *db = db_connection();
    list_t *spells = create_struct_from_db(db, "SELECT * FROM SPELL", create_spell_from_db, sizeof (spell_t));
    print_list(spells, print_spell);
...
char * test_create_stats_from_db() {
    ...
    list_t *stats = create_struct_from_db(db, sql, create_stats_from_db, sizeof (stats_t));
...
char * test_create_inventory_from_db() {
    ...
    list_t *inventory = create_full_inventory_from_db(db, player_id);
...
char *test_get_players_from_db() {
    sqlite3 *db = db_connection();
    array_node_t *players = get_players_from_db(db)->head;
