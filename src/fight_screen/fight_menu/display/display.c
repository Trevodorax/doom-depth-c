...
int display_menu_item(game_window_t *game_window, const char *title, const char *image_path, rect_t container,
    ...
    switch(game_window->ui_type) {
        case CLI:
            return display_menu_item_cli(game_window, title, image_path, container, is_selected);
        case GUI:
            return display_menu_item_gui(game_window, title, image_path, rect_to_SDL_Rect(container), is_selected);
    }
}
